...
def create(numTributes):
    ...
    for i in range(1,numTributes+1):
        ...
        tribNick = tribName + "nickname"
        tribImg = tribName + "img"
        tribGender = tribName + "gender"

        driver.find_element_by_name(tribName).send_keys(cast[i][0])
        driver.find_element_by_name(tribNick).send_keys(cast[i][1])
        driver.find_element_by_name(tribImg).send_keys(cast[i][3])
    ...
    gameUrl = driver.find_element_by_xpath("/html/body/div/div[5]/div[4]/a[1]").text

    print("Game saved successfully! Obtain the url to the game provided by the simulator and enjoy! :)")
    print("Your game link is: " + gameUrl)
...
def main():
    ...
    while numTributes < 1 or numTributes > 3 and validOption == False:
        try:
            ...
            print("Invalid input. Input 1, 2 or 3.")
    print("Please wait. It should not take too long.")
    create(numTributes)
